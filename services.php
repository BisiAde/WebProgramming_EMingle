...
$conn = mysqli_connect($GLOBALS['serverName'], $GLOBALS['dbUserName'], $GLOBALS['dbPassword'], $GLOBALS['dbname']);
...
/*
 * Responsible for login action: Checauk if user supplied email/password pair is in DB
 *
 * @param string email
...
function login($email, $password)
{
	...
	try
	{
		$mysqli = new mysqli($GLOBALS['serverName'], $GLOBALS['dbUserName'], $GLOBALS['dbPassword'], $GLOBALS['dbname']);
    ...
    exit();
		...
		{
			$sqlQuery = $mysqli -> prepare('SELECT user_id, fname, lname from User WHERE email=? AND password=?');
...
function login2($code,$password)
{
    ...
    exit();
		...
		{
			$sqlQuery = $mysqli -> prepare('SELECT user_id, fname, lname from User WHERE code=? AND password=?');
			$sqlQuery -> bind_param('ss', $code, $password );
			$sqlQuery -> execute();
...
function sendEmail($to, $email, $subject='subject', $from='aadenira@cs.odu.edu')
{
